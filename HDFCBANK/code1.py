# This code includes the changes to buy only once and save to orders.json file
import logging
import time
import pandas as pd
import numpy as np
...
instrument_tokens = [341249,738561]
...
def sell_signal():
    ...
    for order in orders:
        ...
        # Fetch and update Nifty historical data if not already fetched
        if nifty_instrument_token not in historical_data_dict:
            historical_data_dict[nifty_instrument_token] = fetch_historical_data(nifty_instrument_token, days=84)
        ...
        if ema_5<ema_13 or ema_5<ema_21 or nifty_ema_5< nifty_ema_13 or nifty_ema_5<nifty_ema_21:
...
#     print('checking exisisting orders')
...
#                         if (instrument_token) not in order_placed:   #if (instrument_token,atm_strike) not in order_placed:
                ...
                if not check_existing_orders(instrument_token):
                    ...
                    if signal:
                        ...
                        if instrument_token not in order_placed:
                            place_order(instrument_token, atm_strike_price, min_quantity, kite.TRANSACTION_TYPE_SELL)
                            place_order(instrument_token, atm_strike_price, min_quantity, kite.TRANSACTION_TYPE_BUY)
                            order_placed[instrument_token] = True
                            print(f"Buy Order Placed for Instrument Token: {instrument_token} at ATM strike price: {atm_strike_price}")
            else:
                # Check if we need to sell
                orders_to_square_off = sell_signal()
...
kws = KiteTicker("5gio34lqmlmn83a5", "JhEq7BUJ5nCWGRgFQntpvlV3ba7fxLFA")
